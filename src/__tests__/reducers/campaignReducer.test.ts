import campaignReducer from '../../redux/slices/campaignSlice';
...
xdescribe('campaignReducer', () => {
	...
	describe('FETCH_CAMPAIGNS_SUCCESS', () => {
		...
		it('define loading como false', () => {
			const state = campaignReducer(undefined, {
		...
		it('limpa error após sucesso', () => {
			...
			const state = campaignReducer(
				initial as any,
				{ type: 'FETCH_CAMPAIGNS_SUCCESS', payload: [] } as any,
			);
			expect(state.error).toBeNull();
		});
	});
	...
	describe('UPDATE_CAMPAIGN', () => {
		...
		it('atualiza a campanha correspondente na lista pelo id', () => {
			...
			const state = campaignReducer(
				initialWithList as any,
				{ type: 'UPDATE_CAMPAIGN', payload: updated } as any,
			);
		...
		it('não altera selected quando o id não bate', () => {
			...
			const updated = { ...mockCampaign, name: 'Nome Atualizado' };
			const state = campaignReducer(
				initialWithOtherSelected as any,
				{ type: 'UPDATE_CAMPAIGN', payload: updated } as any,
			);
	...
	describe('DELETE_CAMPAIGN', () => {
		it('remove a campanha com o id correspondente da lista', () => {
			...
			const state = campaignReducer(
				initial as any,
				{ type: 'DELETE_CAMPAIGN', payload: '1' } as any,
			);
	...
	describe('CREATE_CAMPAIGN', () => {
		it('adiciona nova campanha ao final da lista', () => {
			...
			const state = campaignReducer(
				initial as any,
				{ type: 'CREATE_CAMPAIGN', payload: mockCampaign2 } as any,
			);
